...
export default function CategoryList() {
  ...
  const fetchBrands = async (slug) => {
    ...
    try {
      ...
      const filtered = data
        .filter((b) => b.image && allowedSlugs.includes(b.slug.toLowerCase()))
        .slice(0, 3);
  ...
  return (
    ...
    >
      {img ? (
        <img
          src={img}
          className={`w-10 h-10 object-contain ${isDark ? "brightness-200 invert" : ""}`}
          alt={title}
        />
      ) : isAll ? (
      ...
      <span className={`text-xs font-bold ${isAll ? "text-[#ff7643]" : ""}`}>
        {title}
      </span>
    </button>
  );
...
function SkeletonCard({ theme }) {
  return (
    <div
      className={`w-full h-[120px] rounded-2xl animate-pulse ${theme === "dark" ? "bg-white/[0.05]" : "bg-black/[0.05]"}`}
    />
  );
}
