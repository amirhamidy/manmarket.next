...
const COLOR_URL = "https://api.manmarket.ir/product/v1/color/";
const PAGE_SIZE = 20;
...
const buildProductQuery = ({
  ...
  params.set("in_stock", inStock ?? "unknown");
  params.set("has_discount", hasDiscount ?? "unknown");
  ...
  return `${BASE_URL}/product/?${params.toString()}`;
};
...
export default function CategoryBrandPage() {
  ...
  const fetchPage = useCallback(
    async (
      ...
      try {
        const url = buildProductQuery({
          category: catSlug ?? "",
          brand: brSlug ?? "",
          color: colorId ?? "",
          minPrice: minP ?? "",
          maxPrice: maxP ?? "",
          inStock: inStock ? "true" : "unknown",
  ...
  useEffect(() => {
    ...
    fetchPage(
      ...
      sortOption,
      isPriceActive ? priceRange[0] : "",
      isPriceActive ? priceRange[1] : "",
      showOnlyInStock,
      showOnlyDiscounted
    );
  }, [
  ...
  const handleBrandSelect = (brand) => {
    ...
    if (brand) {
      router.push(
        `/category/${selectedCategory?.slug}/${brand.slug}`
      );
    }
  };
        ...
        return (
          <div className="grid grid-cols-4 gap-3">
            <motion.button
              ...
              className={`col-span-4 flex items-center justify-center py-2.5 px-2 rounded-xl text-[11px] font-medium transition-opacity duration-200 mb-1 ${
                !selectedColor
                  ? `opacity-100 ${
                      isDark
                        ? "bg-white/10"
              ...
              >
                <span
                  className={`w-8 h-8 rounded-full border ${
                    ...
                    color.id ===
                    selectedColor?.id
                      ? "ring-2 ring-[#ff7643] ring-offset-2 ring-offset-transparent"
                      : ""
                  }`}
                  style={{
                    backgroundColor:
    ...
    >
                ...
                {selectedColor ? (
                  ...
                  >
                    <circle
                      ...
                      r="9"
                    />
                    <path d="M12 3a9 9 0 000 18 4.5 4.5 0 100-9 2.25 2.25 0 110-4.5" />
                  </svg>
                )}
            ...
            (product, index) => (
              ...
              >
                <ProductCard
                  product={product}
                  theme={theme}
                  isUnavailable={isProductUnavailable(
                    product
                  )}
                />
              </div>
      ...
      <AnimatePresence mode="wait">
        {!isAnyFilterLoading &&
          totalPages > 1 && (
            <motion.div
              key={`pagination-${selectedCategory?.slug}-${selectedBrand?.slug ?? "all"}-${selectedColor?.id ?? "all"}-${priceRange[0]}-${priceRange[1]}-${showOnlyInStock}-${showOnlyDiscounted}`}
              initial={{
                opacity: 0,
                y: 10,
              }}
