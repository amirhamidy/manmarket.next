...
const BackIcon = ({ color = "#757575" }) => (
  <svg
    ...
    width="22"
    height="13.393"
    viewBox="0 0 16 13.393"
  >
    <path
      d="M18.586,10.121H5.912l4.6-4.6L9.283,4.293l-6.7,6.7,6.7,6.7,1.229-1.229-4.6-4.6H18.586Z"
      transform="translate(-2.586 -4.293)"
      fill={color}
    />
  </svg>
);
...
export default function LoginStep() {
  ...
  const bgColor = theme === "dark" ? "bg-[#000]" : "bg-white";
  const textColor = theme === "dark" ? "text-white" : "text-black";
  const labelBg = theme === "dark" ? "bg-[#000]" : "bg-white";
  const inputBorderDefault =
    theme === "dark" ? "border-gray-600" : "border-[#c7c7c7]";
  const inputBorderFocus = "border-[#ff7643]";

  return (
    <main
      className={`min-h-screen ${bgColor} ${textColor} flex justify-center items-center px-4`}
    >
            ...
            >
              <div className="flex justify-end">
                <button type="button" onClick={() => router.back()}>
                  <BackIcon color={theme === "dark" ? "#ffffff" : "#757575"} />
                </button>
              </div>
              ...
              <div className="relative">
                <input
                  ...
                  className={`w-full h-[48px] px-6 rounded-full border-2 outline-none text-right bg-transparent ${isFloating(number, "number") ? inputBorderFocus : inputBorderDefault} ${textColor}`}
                  maxLength={11}
                />
                <motion.label
                  className={`absolute left-6 px-1 ${labelBg}`}
                  animate={{
                    top: isFloating(number, "number") ? -10 : "50%",
                    translateY: isFloating(number, "number") ? 0 : "-50%",
                    fontSize: isFloating(number, "number") ? "12px" : "14px",
                    color: isFloating(number, "number")
                      ? "#ff7643"
              ...
              <button className="w-full h-[48px] rounded-full bg-[#ff7643] text-white">
                ادامه
              </button>
            </motion.form>
          )}
            ...
            >
              <div className="flex justify-between items-center text-[13px]">
                <div className="space-y-1">
                  {[
                    ...
                    <div
                      key={t}
                      className={`flex justify-start gap-2 ${ok ? "opacity-100" : "opacity-50"}`}
                    >
                      {ok && <CheckIcon />}
              ...
              <div className="relative">
                <input
                  ...
                  onBlur={() => setFocused(null)}
                  className={`w-full h-[48px] px-12 rounded-full border-2 outline-none bg-transparent ${isFloating(password, "password") ? inputBorderFocus : inputBorderDefault} ${textColor}`}
                />
                <motion.label
                  className={`absolute left-6 px-1 ${labelBg}`}
                  animate={{
                    ...
                    fontSize: isFloating(password, "password")
                      ? "12px"
                      : "14px",
                    color: isFloating(password, "password")
                      ? "#ff7643"
                      : "rgba(117,117,117,0.6)",
                  }}
                >
